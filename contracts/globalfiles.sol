...
contract Certificate{
    ...
    function Certificate(address addr) public {
        useraddr = addr;
    }
    ...
    {
         address[] memory certifier = new address[](allrequests.length);
         string[] memory msgg = new string[](allrequests.length);
         string[] memory date = new string[](allrequests.length);

         for (uint i = 0; i < allrequests.length; i++) {
             certifier[i] = allrequests[i].certifier;
             msgg[i] = allrequests[i].msgg;
             date[i] = allrequests[i].date;
         }
         return (certifier,msgg,date);
    }
